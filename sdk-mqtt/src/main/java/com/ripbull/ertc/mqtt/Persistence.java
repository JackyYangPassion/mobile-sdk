...
/**
 * <code>Persistence</code> deals with interacting with the database to persist {@link Connection}
 * objects so created clients survive, the destruction of the singleton {@link Connections} object.
 */
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
  ...
  private static final String SQL_CREATE_SUBSCRIPTION_ENTRIES = "CREATE TABLE "
      + TABLE_SUBSCRIPTIONS
      + " ("
      + _ID
      + " INTEGER PRIMARY KEY,"
  ...
  private ContentValues getValues(Connection connection) {
    ...
    values.put(COLUMN_CLEAN_SESSION,
        conOpts.isCleanSession() ? 1 : 0); // convert boolean to int and then put in values
    values.put(COLUMN_PASSWORD,
        password != null ? String.valueOf(password) : null); // convert char[] to String
    values.put(COLUMN_MESSAGE,
        lastWill != null ? new String(lastWill.getPayload()) : null); // convert byte[] to string
  ...
  public long persistSubscription(Subscription subscription) throws PersistenceException {
    ...
    values.put(SUBSCRIPTIONS_COLUMN_NOTIFY, subscription.isEnableNotifications() ? 1
        : 0); // convert boolean to int and then put in values
  ...
  public void deleteSubscription(Subscription subscription) {
    ...
    SQLiteDatabase db = getWritableDatabase();
    db.delete(TABLE_SUBSCRIPTIONS, _ID + "=?",
        new String[] { String.valueOf(subscription.getPersistenceId()) });
    db.close();
  ...
  public List<Connection> restoreConnections(Context context) throws PersistenceException {
    ...
    String[] connectionColumns = {
        COLUMN_CLIENT_HANDLE, COLUMN_HOST, COLUMN_port, COLUMN_client_ID, COLUMN_ssl,
        COLUMN_KEEP_ALIVE, COLUMN_CLEAN_SESSION, COLUMN_TIME_OUT, COLUMN_USER_NAME, COLUMN_PASSWORD,
        COLUMN_TOPIC, COLUMN_MESSAGE, COLUMN_RETAINED, COLUMN_QOS, _ID
    };

    // Columns to return for subscription
    String[] subscriptionColumns = {
        COLUMN_CLIENT_HANDLE, SUBSCRIPTIONS_COLUMN_TOPIC, SUBSCRIPTIONS_COLUMN_NOTIFY,
        SUBSCRIPTIONS_COLUMN_QOS, _ID
    };
    ...
    for (int i = 0; i < c.getCount(); i++) {
      if (!c.moveToNext()) { // move to the next item throw persistence exception, if it fails
        throw new PersistenceException(
            "Failed restoring connection - count: " + c.getCount() + "loop iteration: " + i);
      }
      ...
      // get all values that need converting and convert integers to booleans in line using
      ...
      connection = Connection.createConnection(clientHandle, clientID, host, context, ssl);
      ...
      Cursor sub_c =
          db.query(TABLE_SUBSCRIPTIONS, subscriptionColumns, subscriptionWhereQuery, args, null,
              null, sort);
      ...
      for (int x = 0; x < sub_c.getCount(); x++) {
        if (!sub_c.moveToNext()) { // move to the next item throw persistence exception, if it fails
          throw new PersistenceException("Failed restoring subscription - count: "
              + sub_c.getCount()
              + "loop iteration: "
              + x);
        }
        Long sub_id = sub_c.getLong(sub_c.getColumnIndexOrThrow(_ID));
        String sub_clientHandle =
            sub_c.getString(sub_c.getColumnIndexOrThrow(COLUMN_CLIENT_HANDLE));
        String sub_topic = sub_c.getString(sub_c.getColumnIndexOrThrow(SUBSCRIPTIONS_COLUMN_TOPIC));
        boolean sub_notify =
            sub_c.getInt(sub_c.getColumnIndexOrThrow(SUBSCRIPTIONS_COLUMN_NOTIFY)) == 1;
  ...
  public void deleteConnection(Connection connection) {
    ...
    db.delete(TABLE_CONNECTIONS, _ID + "=?",
        new String[] { String.valueOf(connection.persistenceId()) });
    db.close();
    // don't care if it failed, means it's not in the db therefore no need to delete
