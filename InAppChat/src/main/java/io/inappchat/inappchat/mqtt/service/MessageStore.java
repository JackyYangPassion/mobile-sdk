 ...
 *
 * <ul>
 *   <li>A Service should store messages as they arrive via {@link #storeArrived(String, String,
 *       MqttMessage)}.
 *   <li>When a message has been passed to the consuming entity, {@link #discardArrived(String,
 *       String)} should be called.
 *   <li>To recover messages which have not been definitely passed to the consumer, {@link
 *       MessageStore#getAllArrivedMessages(String)} is used.
...
interface MessageStore {
  ...
  /**
   * Discard a message - called when we are certain that an arrived message has reached the
   * application.
   *
   * @param clientHandle identifier for the client
   ...
   *
   * @param clientHandle identifier for the client - if null, then messages for all clients are
   * returned
   */
