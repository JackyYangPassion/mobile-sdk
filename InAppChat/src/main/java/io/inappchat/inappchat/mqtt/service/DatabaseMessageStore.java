...
class DatabaseMessageStore implements MessageStore {
  ...
  private static final String TAG = Constants.TAG + " : DatabaseMessageStore";
  ...
  private static class MQTTDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase database) {
      String createArrivedTableStatement = "CREATE TABLE "
          ...
          + " TEXT, "
          + MqttServiceConstants.DESTINATION_NAME
          + " TEXT, "
          + MqttServiceConstants.PAYLOAD
          + " BLOB, "
          + MqttServiceConstants.QOS
          + " INTEGER, "
          + MqttServiceConstants.RETAINED
          + " TEXT, "
          + MqttServiceConstants.DUPLICATE
          + " TEXT, "
          + MTIMESTAMP
          + " INTEGER"
          + ");";
  ...
  public String storeArrived(String clientHandle, String topic, MqttMessage message) {
    ...
    traceHandler.traceDebug(TAG,
        "storeArrived{" + clientHandle + "}, {" + message.toString() + "}");
  ...
  private int getArrivedRowCount(String clientHandle) {
    ...
    Cursor c = db.query(ARRIVED_MESSAGE_TABLE_NAME, // Table Name
  ...
  public boolean discardArrived(String clientHandle, String id) {
    ...
    if (rows != 1) {
      traceHandler.traceError(TAG, "discardArrived - Error deleting message {"
          + id
    ...
    return new Iterator<StoredMessage>() {
...

      {
        ...
        if (clientHandle == null) {
          ...
          c = db.query(ARRIVED_MESSAGE_TABLE_NAME, null, MqttServiceConstants.CLIENT_HANDLE + "=?",
              selectionArgs, null, null, "mtimestamp ASC");
        }
      ...
      public StoredMessage next() {
        ...
        boolean retained =
            Boolean.parseBoolean(c.getString(c.getColumnIndex(MqttServiceConstants.RETAINED)));
        boolean dup =
            Boolean.parseBoolean(c.getString(c.getColumnIndex(MqttServiceConstants.DUPLICATE)));
  ...
  public void clearArrivedMessages(String clientHandle) {
    ...
    if (clientHandle == null) {
      ...
      traceHandler.traceDebug(TAG,
          "clearArrivedMessages: clearing the table of " + clientHandle + " messages");
      rows = db.delete(ARRIVED_MESSAGE_TABLE_NAME, MqttServiceConstants.CLIENT_HANDLE + "=?",
          selectionArgs);
    }
