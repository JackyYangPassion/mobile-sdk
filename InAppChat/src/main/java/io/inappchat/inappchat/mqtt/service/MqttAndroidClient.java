package io.inappchat.inappchat.mqtt.service;
...
public class MqttAndroidClient extends BroadcastReceiver implements IMqttAsyncClient {
  ...
  /**
   * The Acknowledgment mode for messages received from {@link MqttCallback#messageArrived(String,
   * MqttMessage)}
   */
  public enum Ack {
    /**
    ...
    AUTO_ACK,
    /**
     * When {@link MqttCallback#messageArrived(String, MqttMessage)} returns, the message will not
     * be acknowledged as received, the application will have to make an acknowledgment call to
     * {@link MqttAndroidClient} using {@link MqttAndroidClient#acknowledgeMessage(String)}
     */
    MANUAL_ACK
  }

  private static final String SERVICE_NAME = "io.inappchat.inappchat.mqtt.service.MqttService";
  ...
  private final class MyServiceConnection implements ServiceConnection {
    ...
    public void onServiceConnected(ComponentName name, IBinder binder) {
      Log.d(Constants.TAG, "MyServiceConnection.class : onServiceConnected");
    ...
    public void onServiceDisconnected(ComponentName name) {
      Log.d(Constants.TAG, "MyServiceConnection.class : onServiceDisconnected");
  ...
  public MqttAndroidClient(Context ctx, String serverURI, String clientId,
   ...
   *
   * <p>All clients connected to the same server or server farm must have a unique ID.
   *
  ...
  /**
   * Close the client. Releases all resource associated with the client. After the client has been
   * closed it cannot be reused. For instance attempts to connect will fail.
   */
  @Override
  public void close() {
    if (mqttService != null) {
      if (clientHandle == null) {
        clientHandle =
            mqttService.getClient(serverURI, clientId, myContext.getApplicationInfo().packageName,
                persistence);
      }
   ...
   *
   * @return token used to track and wait for the connect to complete. The token will be passed to
   * the callback methods if a callback is set.
   ...
   *
   * <p>The connection will be established using the options specified in the {@link
   * MqttConnectOptions} parameter.
   *
   * @param options a set of connection parameters that override the defaults.
   * @return token used to track and wait for the connect to complete. The token will be passed to
   * any callback that has been set.
   * @throws MqttException for any connected problems
   * @see #connect(MqttConnectOptions, Object, IMqttActionListener)
   */
  @Override
   ...
   * <p>The default options are specified in {@link MqttConnectOptions} class.
   *
   * @param userContext optional object used to pass context to the callback. Use null if not
   * required.
   * @param callback optional listener that will be notified when the connect completes. Use null if
   * not required.
   * @return token used to track and wait for the connect to complete. The token will be passed to
   * any callback that has been set.
   * @throws MqttException for any connected problems
   ...
   *
   * <p>The server to connect to is specified on the constructor. It is recommended to call {@link
   * #setCallback(MqttCallback)} prior to connecting in order that messages destined for the client
   * can be accepted as soon as the client is connected.
   *
   * <p>The method returns control before the connect completes. Completion can be tracked by:
   *
   * <ul>
   *   <li>Waiting on the returned token {@link IMqttToken#waitForCompletion()} or
   *   <li>Passing in a callback {@link IMqttActionListener}
   * </ul>
   *
   * @param options a set of connection parameters that override the defaults.
   * @param userContext optional object for used to pass context to the callback. Use null if not
   * required.
   * @param callback optional listener that will be notified when the connect completes. Use null if
   * not required.
   * @return token used to track and wait for the connect to complete. The token will be passed to
    ...
    if (mqttService == null) { // First time - must bind to the service
      ...
      if (service == null) {
        ...
        if (listener != null) {
          ...
          Log.d(Constants.TAG, "Cannot start service..");
        }
      }
   ...
   *
   * <p>An attempt is made to quiesce the client allowing outstanding work to complete before
   * disconnecting. It will wait for a maximum of 30 seconds for work to quiesce before
   * disconnecting. This method must not be called from inside {@link MqttCallback} methods.
   *
   * @return token used to track and wait for disconnect to complete. The token will be passed to
   * any callback that has been set.
   ...
   *
   * <p>An attempt is made to quiesce the client allowing outstanding work to complete before
   * disconnecting. It will wait for a maximum of the specified quiesce time for work to complete
   * before disconnecting. This method must not be called from inside {@link MqttCallback} methods.
   *
   * @param quiesceTimeout the amount of time in milliseconds to allow for existing work to finish
   * before disconnecting. A value of zero or less means the client will not quiesce.
   * @return token used to track and wait for disconnect to complete. The token will be passed to
   * the callback methods if a callback is set.
   * @throws MqttException for problems encountered while disconnecting
   * @see #disconnect(long, Object, IMqttActionListener)
   ...
   * @param userContext optional object used to pass context to the callback. Use null if not
   * required.
   * @param callback optional listener that will be notified when the disconnect completes. Use null
   * if not required.
   * @return token used to track and wait for the disconnect to complete. The token will be passed
   * to any callback that has been set.
   * @throws MqttException for problems encountered while disconnecting
   * @see #disconnect(long, Object, IMqttActionListener)
   */
  @Override
   ...
   * Disconnects from the server.
   *
   * <p>The client will wait for {@link MqttCallback} methods to complete. It will then wait for up
   * to the quiesce timeout to allow for work which has already been initiated to complete. For
   * instance when a QoS 2 message has started flowing to the server but the QoS 2 flow has not
   * completed.It prevents new messages being accepted and does not send any messages that have been
   * accepted but not yet started delivery across the network to the server. When work has completed
   * or after the quiesce timeout, the client will disconnect from the server. If the cleanSession
   * flag was set to false and next time it is also set to false in the connection, the messages
   * made in QoS 1 or 2 which were not previously delivered will be delivered this time.
   *
   * <p>This method must not be called from inside {@link MqttCallback} methods.
   *
   * <p>The method returns control before the disconnect completes. Completion can be tracked by:
   *
   * <ul>
   *   <li>Waiting on the returned token {@link IMqttToken#waitForCompletion()} or
   *   <li>Passing in a callback {@link IMqttActionListener}
   ...
   *
   * <p>A convenience method, which will create a new {@link MqttMessage} object with a byte array
   * payload and the specified QoS, and then publish it.
   *
   ...
   * @return token used to track and wait for the publish to complete. The token will be passed to
   * any callback that has been set.
   * @throws MqttPersistenceException when a problem occurs storing the message
   * @throws IllegalArgumentException if value of QoS is not 0, 1 or 2.
   * @throws MqttException for other errors encountered while publishing the message. For instance,
  ...
  /**
   * Publishes a message to a topic on the server. Takes an {@link MqttMessage} message and delivers
   * it to the server at the requested quality of service.
   *
   * @param topic to deliver the message to, for example "finance/stock/ibm".
   ...
   *
   * <p>A convenience method, which will create a new {@link MqttMessage} object with a byte array
   * payload, the specified QoS and retained, then publish it.
   *
   * @param topic to deliver the message to, for example "finance/stock/ibm".
   * @param payload the byte array to use as the payload
   * @param qos the Quality of Service to deliver the message at. Valid values are 0, 1 or 2.
   ...
   * @param userContext optional object used to pass context to the callback. Use null if not
   * required.
   * @param callback optional listener that will be notified when message delivery has completed to
  ...
  public IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained,
    ...
    MqttDeliveryTokenAndroid token =
        new MqttDeliveryTokenAndroid(this, userContext, callback, message);
    String activityToken = storeToken(token);
    ...
    if (mqttService != null && clientHandle != null) {
      IMqttDeliveryToken internalToken =
          mqttService.publish(clientHandle, topic, payload, qos, retained, null, activityToken);
   ...
   * Publishes a message to a topic on the server.
   *
   * <p>Once this method has returned cleanly, the message has been accepted for publication by the
   * client and will be delivered on a background thread. In the event the connection fails or the
   * client stops, Messages will be delivered to the requested quality of service once the
   * connection is re-established to the server on condition that:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>When building an application, the design of the topic tree should take into account the
   ...
   *   <li>Do not include the null character (Unicode <em>\x0000</em>) in any topic.
   * </ul>
   *
   ...
   * </ul>
   *
   * <p>The method returns control before the publish completes. Completion can be tracked by:
   *
   * <ul>
   *   <li>Setting an {@link IMqttAsyncClient#setCallback(MqttCallback)} where the {@link
   *       MqttCallback#deliveryComplete(IMqttDeliveryToken)} method will be called.
   *   <li>Waiting on the returned token {@link MqttToken#waitForCompletion()} or
   *   <li>Passing in a callback {@link IMqttActionListener} to this method
   * </ul>
   *
   * @param topic to deliver the message to, for example "finance/stock/ibm".
   * @param message to deliver to the server
  ...
  public IMqttDeliveryToken publish(String topic, MqttMessage message, Object userContext,
    ...
    String activityToken = storeToken(token);
    IMqttDeliveryToken internalToken =
        mqttService.publish(clientHandle, topic, message, null, activityToken);
    token.setDelegate(internalToken);
    return token;
  }
   ...
   * @param qos the maximum quality of service at which to subscribe. Messages published at a lower
   ...
   * @return token used to track and wait for the subscribe to complete. The token will be passed to
   * callback methods if set.
   ...
   *
   * <p>Provides an optimized way to subscribe to multiple topics compared to subscribing to each
   * one individually.
   *
   ...
   * quality of service will be received at the published QoS. Messages published at a higher
   * quality of service will be received using the QoS specified on the subscription.
   * @return token used to track and wait for the subscription to complete. The token will be passed
   * to callback methods if set.
   * @throws MqttSecurityException for security related problems
   * @throws MqttException for non security related problems
   * @see #subscribe(String[], int[], Object, IMqttActionListener)
   */
   ...
   * one individually.
   *
   * <p>The {@link #setCallback(MqttCallback)} method should be called before this method, otherwise
   * any received messages will be discarded.
   *
   * <p>If (@link MqttConnectOptions#setCleanSession(boolean)} was set to true, when connecting to
   * the server, the subscription remains in place until either:
   *
   * <ul>
   *   <li>The client disconnects
   *   <li>An unsubscribe method is called to unsubscribe the topic
   * </ul>
   *
   * <p>If (@link MqttConnectOptions#setCleanSession(boolean)} was set to false, when connecting to
   * the server, the subscription remains in place until either:
   *
   * <ul>
   *   <li>An unsubscribe method is called to unsubscribe the topic
   ...
   * </ul>
   *
   * <p>With cleanSession set to false the MQTT server will store messages on behalf of the client
   * when the client is not connected. The next time the client connects with the <b>same client
   * ID</b> the server will deliver the stored messages to the client.
   *
   * <p>The "topic filter" string is used when subscription may contain special characters, which
   * allows you to subscribe to multiple topics at once.
   *
   * <dl>
   *   <dt>Topic level separator
   *   <dd>The forward slash (/) is used to separate each level within a topic tree and provide a
   *       hierarchical structure to the topic space. The use of the topic level separator is
   *       significant when the two wildcard characters are encountered in topics specified by
   *       subscribers.
   *   <dt>Multi-level wildcard
   *   <dd>
   *       <p>The number sign (#) is a wildcard character that matches any number of levels within a
   *       topic. For example, if you subscribe to <span><span
   ...
   *       </ul>
   *       <p>The multi-level wildcard can represent zero or more levels. Therefore,
   *       <em>finance/#</em> can also match the singular <em>finance</em>, where <em>#</em>
   *       represents zero levels. The topic level separator is meaningless in this context, because
   *       there are no levels to separate.
   *       <p>The <span>multi-level</span> wildcard can be specified only on its own or next to the
   *       topic level separator character. Therefore, <em>#</em> and <em>finance/#</em> are both
   *       valid, but <em>finance#</em> is not valid. <span>The multi-level wildcard must be the
   *       last character used within the topic tree. For example, <em>finance/#</em> is valid but
   *       <em>finance/#/closingprice</em> is not valid.</span>
   *   <dt>Single-level wildcard
   *   <dd>
   *       <p>The plus sign (+) is a wildcard character that matches only one topic level. For
   *       example, <em>finance/stock/+</em> matches <em>finance/stock/ibm</em> and
   *       <em>finance/stock/xyz</em>, but not <em>finance/stock/ibm/closingprice</em>. Also,
   *       because the single-level wildcard matches only a single level, <em>finance/+</em> does
   *       not match <em>finance</em>.
   *       <p>Use the single-level wildcard at any level in the topic tree, and in conjunction with
   *       the multilevel wildcard. Specify the single-level wildcard next to the topic level
   *       separator, except when it is specified on its own. Therefore, <em>+</em> and
   *       <em>finance/+</em> are both valid, but <em>finance+</em> is not valid. <span>The
   *       single-level wildcard can be used at the end of the topic tree or within the topic tree.
   *       For example, <em>finance/+</em> and <em>finance/+/ibm</em> are both valid.</span>
   * </dl>
   *
   * <p>The method returns control before the subscribe completes. Completion can be tracked by:
   *
   * <ul>
   *   <li>Waiting on the supplied token {@link MqttToken#waitForCompletion()} or
   *   <li>Passing in a callback {@link IMqttActionListener} to this method
   * </ul>
   *
   * @param topic one or more topics to subscribe to, which can include wildcards
   * @param qos the maximum quality of service to subscribe each topic at.Messages published at a
   * lower quality of service will be received at the published QoS. Messages published at a
   * higher quality of service will be received using the QoS specified on the subscription.
   * @param userContext optional object used to pass context to the callback. Use null if not
   * required.
   * @param callback optional listener that will be notified when subscribe has completed
   * @return token used to track and wait for the subscribe to complete. The token will be passed to
  ...
  public IMqttToken subscribe(String topicFilter, int qos, Object userContext,
      IMqttActionListener callback, IMqttMessageListener messageListener) throws MqttException {
  ...
  public IMqttToken subscribe(String[] topicFilters, int[] qos, Object userContext,
      IMqttActionListener callback, IMqttMessageListener[] messageListeners) throws MqttException {
   ...
   *
   * @param topic the topic to unsubscribe from. It must match a topic specified on an earlier
   * subscribe.
   * @return token used to track and wait for the unsubscribe to complete. The token will be passed
   * to callback methods if set.
   ...
   *
   * @param topic one or more topics to unsubscribe from. Each topic must match one specified on an
   ...
   * Requests the server to unsubscribe the client from one or more topics.
   *
   * <p>Unsubcribing is the opposite of subscribing. When the server receives the unsubscribe
   * request it looks to see if it can find a matching subscription for the client and then removes
   * it. After this point the server will send no more messages to the client for this subscription.
   *
   * <p>The topic(s) specified on the unsubscribe must match the topic(s) specified in the original
   * subscribe request for the unsubscribe to succeed
   *
   * <p>The method returns control before the unsubscribe completes. Completion can be tracked by:
   *
   * <ul>
   *   <li>Waiting on the returned token {@link MqttToken#waitForCompletion()} or
   *   <li>Passing in a callback {@link IMqttActionListener} to this method
   ...
   *
   * <p>If a client has been restarted and there are messages that were in the process of being
   * delivered when the client stopped, this method returns a token for each in-flight message to
   * enable the delivery to be tracked. Alternately the {@link
   * MqttCallback#deliveryComplete(IMqttDeliveryToken)} callback can be used to track the delivery
   * of outstanding messages.
   *
   * <p>If a client connects with cleanSession true then there will be no delivery tokens as the
   * cleanSession option deletes all earlier state. For state to be remembered the client must
   ...
   *
   * <p>There are a number of events that the listener will be notified about. These include:
   *
   * <ul>
   *   <li>A new message has arrived and is ready to be processed
   *   <li>The connection to the server has been lost
   *   <li>Delivery of a message to the server has completed
   * </ul>
   *
   * <p>Other events that track the progress of an individual operation such as connect and
   * subscribe can be tracked using the {@link MqttToken} returned from each non-blocking method or
   * using setting a {@link IMqttActionListener} on the non-blocking method.
   *
   * <p>
   *
   ...
   *
   * @param traceEnabled set <code>true</code> to enable trace, otherwise, set <code>false</code> to
   * disable trace
   */
  public void setTraceEnabled(boolean traceEnabled) {
    this.traceEnabled = traceEnabled;
    if (mqttService != null) mqttService.setTraceEnabled(traceEnabled);
  }

  /**
   * Process incoming Intent objects representing the results of operations and asynchronous
   * activities such as message received
   *
   * <p><strong>Note:</strong> This is only a public method because the Android APIs require such.
   * <br>
   ...
   *
   * @param messageId the messageId received from the MqttMessage (To access this field you need to
   * cast {@link MqttMessage} to {@link ParcelableMqttMessage})
  ...
  private void messageArrivedAction(Bundle data) {
    if (callback != null) {
      ...
      ParcelableMqttMessage message =
          data.getParcelable(MqttServiceConstants.CALLBACK_MESSAGE_PARCEL);
      try {
   ...
   * @return an identifier for the token which can be passed to the Android Service
   */
   ...
   *
   * <p>A convenience method, which will help user to create a SSLSocketFactory object
   *
   * @param keyStore the SSL key store which is generated by some SSL key tool, such as keytool in
   * Java JDK
  ...
  /**
   * Register receiver to receiver intent from MqttService. Call this method when activity is hidden
   * and become to show again.
   *
