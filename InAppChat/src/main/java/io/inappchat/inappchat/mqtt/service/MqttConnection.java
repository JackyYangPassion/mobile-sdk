...
/**
 * MqttConnection holds a MqttAsyncClient {host,port,clientId} instance to perform MQTT operations
 * to MQTT broker.
 *
 * <p>Most of the major API here is intended to implement the most general forms of the methods in
 * IMqttAsyncClient, with slight adjustments for the Android environment<br>
 * These adjustments usually consist of adding two parameters to each method :-
 *
 * <ul>
 *   <li>invocationContext - a string passed from the application to identify the context of the
 *       operation (mainly included for support of the javascript API implementation)
 *   <li>activityToken - a string passed from the Activity to relate back to a callback method or
 *       other context-specific data
 * </ul>
 *
 * <p>Operations are very much asynchronous, so success and failure are notified by packing the
 * relevant data into Intent objects which are broadcast back to the Activity via the
 * MqttService.callbackToActivity() method.
 */
class MqttConnection implements MqttCallbackExtended {

  // Strings for Intents etc..
  private static final String TAG = Constants.TAG + " : MqttConnection";
  ...
  public void connect(MqttConnectOptions options, String invocationContext, String activityToken) {
    ...
    Log.i(TAG, "Connecting {" + serverURI + "} as {" + clientId + "}");
    ...
    try {
      ...
      // if myClient is null, then create a new connection
        ...
        Log.d(TAG, "MqttAsyncClient is null... Creating new one ");
  ...
  private void doAfterConnectSuccess(final Bundle resultBundle) {
    ...
    Log.d(TAG, "doAfterConnectSuccess: *****");
    acquireWakeLock();
  ...
  /**
   * Attempt to deliver any outstanding messages we've received but which the application hasn't
   * acknowledged. If "cleanSession" was specified, we'll have already purged any such messages from
   * our messageStore.
   */
  private void deliverBacklog() {
    Iterator<MessageStore.StoredMessage> backlog =
        service.messageStore.getAllArrivedMessages(clientHandle);
   ...
   *
   * @param messageId the message's identifier in the messageStore, so that a callback can be made
   * to remove it once delivered
  ...
  public IMqttDeliveryToken publish(String topic, MqttMessage message, String invocationContext,
    ...
    if ((myClient != null) && (myClient.isConnected())) {
      ...
      try {
        ...
        && (this.bufferOpts.isBufferEnabled())) {
   ...
   * @param activityToken arbitrary identifier to be passed back to the Activity
   */
  public void subscribe(final String topic, final int qos, String invocationContext,
      String activityToken) {
    service.traceDebug(TAG, "subscribe({"
        + topic
        + "},"
        + qos
        + ",{"
  ...
  void unsubscribe(final String topic, String invocationContext, String activityToken) {
    service.traceDebug(TAG,
        "unsubscribe({" + topic + "},{" + invocationContext + "}, {" + activityToken + "})");
    final Bundle resultBundle = new Bundle();
    resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION,
   ...
   */
  void unsubscribe(final String[] topic, String invocationContext, String activityToken) {
    service.traceDebug(TAG, "unsubscribe({"
        + Arrays.toString(topic)
        + "},{"
        + invocationContext
        + "}, {"
  ...
  public void connectionLost(Throwable why) {
    ...
    if (why != null) {
      ...
      resultBundle.putString(MqttServiceConstants.CALLBACK_EXCEPTION_STACK,
  ...
  /**
   * Callback to indicate a message has been delivered (the exact meaning of "has been delivered" is
   * dependent on the QOS value)
   *
  ...
  public void deliveryComplete(IMqttDeliveryToken messageToken) {
    ...
    // this notification will have kept the connection alive but send the previously sechudled ping
    // anyway
  }
  ...
  /**
   * Store details of sent messages so we can handle "deliveryComplete" callbacks from the
   * mqttClient
   */
  private void storeSendDetails(final String topic, final MqttMessage msg,
      final IMqttDeliveryToken messageToken, final String invocationContext,
   ...
   *
   * <p>Simply handles the basic success/failure cases for operations which don't return results
   */
  ...
  /**
   * Reconnect<br>
   * Only appropriate if cleanSession is false and we were connected. Declare as synchronized to
   * avoid multiple calls to this method to send connect multiple times
   */
    ...
    if (connectOptions.isAutomaticReconnect()) {
      ...
      try {
        ...
        */
        ...
        setConnectingState(false);
        MqttException newEx =
            new MqttException(MqttException.REASON_CODE_UNEXPECTED_ERROR, e.getCause());
