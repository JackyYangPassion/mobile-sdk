  ...
  companion object {
    ...
    fun sendE2EMessage(
      ...
      if (singleChat.parentMsgId != null && singleChat.parentMsgId!!.isNotEmpty()) {
        val singleChatParent = data.db().singleChatDao().getChatByLocalMsgId(singleChat.parentMsgId, singleChat.threadId)
      ...
      val e2eRequest = ChatRequestMapper.requestE2E(
        thread!!, message, singleChat.id, replyThread, eKeyTable, eKeyDao, data.db().groupDao(),
        null, parallelDeviceList, singleChat.customData
      )
      ...
      disposable.add(data.network().api().sendE2EMessage(
        ...
        if (chatStatus == null || !chatStatus.retryRequired) {
            ...
            for (e2eKey in chatStatus.keyList!!) {
              if (e2eKey.returnCode != null && e2eKey.returnCode.equals("receiverKeyNotValid")) {
                ...
                if (e2eKey.deviceId == deviceId) {
                  eKeyDao.setKeyId(
                    chatUserId, e2eKey.publicKey, e2eKey.keyId, eKeyTable.time, deviceId
                  )
                } else {
          ...
          if (response.replyThreadFeatureData?.parentMsgId != null && response.replyThreadFeatureData?.parentMsgId!!.isNotEmpty()) {
            ...
            val chatThread = chatThreadDao.getChatByClientId(msgId1, singleChat.threadId)
            chatThread.msgUniqueId = response.msgUniqueId
            chatThread.status = MessageStatus.SENT.status
            Logger.d("thread_debug ###", "API : METADATA_ID : $msgId1 ::: Thread_Id : $singleChat.threadId")
            ...
            if (response.replyThreadFeatureData?.replyMsgConfig == 1) {
              val singleChat = singleChatDao.getChatByLocalMsgId(msgId1, singleChat.threadId)
            ...
            singleChatDao.getChatByClientIdByRxSingle(msgId1, singleChat.threadId).flatMap { singleChat ->
              singleChat.msgUniqueId = response.msgUniqueId
              singleChat.status = MessageStatus.SENT.status
              singleChatDao.insertWithReplace(singleChat)
                ...
                if (chatStatus.keyList!!.isNotEmpty()) {
                  ...
                  for (e2eKey in chatStatus.keyList!!) {
                    if (e2eKey.returnCode != null && e2eKey.returnCode.equals("receiverKeyNotValid")) {
                      ...
                      if (e2eKey.deviceId == deviceId) {
                        Log.i("Offline", "receiverKeyValidationError - 2")
                        eKeyDao.setKeyId(
                          chatUserId,
                          e2eKey.publicKey,
                          e2eKey.keyId,
                        ...
                        Log.i("Offline", "receiverKeyValidationError - 3")
