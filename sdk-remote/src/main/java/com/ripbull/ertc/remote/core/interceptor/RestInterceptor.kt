package com.ripbull.ertc.remote.core.interceptor


import com.ripbull.ertc.remote.Constants
import com.ripbull.ertc.remote.NetworkConfig
import com.ripbull.ertc.remote.core.ErtcException
...
//          }
        ...
        throw when (ex) {
          ...
          is ConnectException, is java.net.UnknownHostException -> {
            ...
            ErtcException(ErtcException.Error.NoNetwork(Constants.HTTP_CODE_NETWORK), "Network unavailable", ex)
          }
  ...
  private fun addAuthToken(serverType: Int, builder: Request.Builder) {
    var token = ""
    if (serverType == serverUser && nwConfig.userToken.isNotEmpty()) {
      token = nwConfig.userToken
    } else if (serverType == serverChat && nwConfig.chatToken.isNotEmpty()) {
