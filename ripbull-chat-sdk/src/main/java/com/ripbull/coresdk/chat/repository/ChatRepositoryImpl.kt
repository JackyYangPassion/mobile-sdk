...
import com.ripbull.coresdk.chat.mapper.ChatEntityMapper.getChatThreadRow
import com.ripbull.coresdk.chat.mapper.ChatEntityMapper.transform
...
import com.ripbull.coresdk.chat.mapper.ChatRequestMapper.mentions
import com.ripbull.coresdk.chat.mapper.ChatRequestMapper.request
import com.ripbull.coresdk.chat.mapper.ChatRequestMapper.requestE2E
import com.ripbull.coresdk.chat.mapper.ChatRequestMapper.requestEditE2E
...
import com.ripbull.coresdk.core.type.MessageStatus.SENT
...
import com.ripbull.coresdk.utils.Constants.TenantConfig
import com.ripbull.coresdk.utils.CrashReportingObserver
import com.ripbull.coresdk.utils.DisposableList
import com.ripbull.coresdk.utils.Logger
import com.ripbull.coresdk.utils.Utils
import com.ripbull.coresdk.wrappers.UserWrapper
...
import com.ripbull.ertc.remote.model.request.MessageList
...
import com.ripbull.ertc.remote.model.request.SearchQuery
...
class ChatRepositoryImpl private constructor(
  ...
  companion object {
    ...
    fun newInstance(
      ...
      val remote: ChatRemoteRepository = ChatRemoteRepository.newInstance(dataManager, threadRepository, eventHandler, downloadRepository)
  ...
  override fun getMessages(
    ...
    return singleChatDao.hasThreadMessage(threadId).flatMap {
      if (preference.skipRestoreFlag) {
        return@flatMap local.getMessages(tenantId, threadId, currentMsgId, direction, pageSize, isGlobalSearched)
      } else if (it.size > 1) {
        if (currentMsgId.isNullOrEmpty()) {
          return@flatMap local.getMessages(tenantId, threadId, currentMsgId, direction, pageSize, isGlobalSearched)
        } else if (pageSize != null && pageSize > 0) {
          return@flatMap remote.getMessages(tenantId, threadId, currentMsgId, direction, pageSize, isGlobalSearched)
        }
        return@flatMap local.getMessages(tenantId, threadId, currentMsgId, direction, pageSize, isGlobalSearched)
      } else {
        return@flatMap remote.getMessages(tenantId, threadId, currentMsgId, direction, pageSize, isGlobalSearched)
    ...
    return Single.create { e: SingleEmitter<Thread?> ->
      ...
      if (message.parentMsgId != null && message.parentMsgId.isNotEmpty()) {
        ...
        if (message.sendToChannel == 1) {
          ...
          val chatThread = getChatThreadRow(thread, message, msgId, message.parentMsgId, customData = customData)
          chatThread.parentMsgId = id
          chatThread.sendToChannel = sendToChannel
          chatThreadDao.insertWithReplace(chatThread)

          val chatRow = getChatRow(thread, message, msgId, id, getParentMessage(singleChat), customData = customData)
        ...
        singleChatDao.insertWithReplace(getChatRow(thread, message, msgId, customData = customData))
      }
      ...
      response = when {
        ...
        message.file != null  -> {
          ...
          networkManager.api()
            .sendMessage(
              tenantId, request(
                thread!!, message, msgId,
                replyThread, getMentionedChatUserIds(message), customData = customData
      ...
      response.flatMap { messageResponse ->
        ...
        if (messageResponse.replyThreadFeatureData?.parentMsgId != null && messageResponse.replyThreadFeatureData?.parentMsgId!!.isNotEmpty()) {
          ...
          Logger.d("thread_debug ###", "API : METADATA_ID : $msgId1 ::: Thread_Id : $threadId")
          ...
          singleChatDao.getChatByClientIdByRxSingle(msgId1, threadId).flatMap { singleChat ->
            singleChat.msgUniqueId = messageResponse.msgUniqueId
            singleChat.status = SENT.status
            if (messageResponse.media != null) {
              singleChat.mediaName = messageResponse.media?.name
        ...
        it.cause?.let { cause ->
          if (cause.toString() == "java.net.SocketTimeoutException: timeout") {
            if (message.media != null || message.file != null) {
              ...
              if (message.parentMsgId != null && message.parentMsgId.isNotEmpty()) {
                return@onErrorReturn transform(chatThread = chatThreadDao.getChatByClientId(msgId, threadId))
              } else {
                return@onErrorReturn transform(singleChat = singleChatDao.getChatByLocalMsgId(msgId, threadId))
              }
            }
          }
        }
    ...
    return Single.create { e: SingleEmitter<Thread?> ->
      ...
      if (message.parentMsgId != null && message.parentMsgId.isNotEmpty()) {
        ...
        if (message.sendToChannel == 1) {
          ...
          val chatThread =
            getChatThreadRow(thread, message, msgId, message.parentMsgId, customData = customData)
          chatThread.parentMsgId = id
          chatThread.sendToChannel = sendToChannel
          chatThreadDao.insertWithReplace(chatThread)
      ...
      val e2eRequest = requestE2E(
        thread!!, message, msgId, replyThread, eKeyTable, ekeyDao, dataManager.db().groupDao(),
        getMentionedChatUserIds(message), parallelDeviceList, customData
      )
      ...
      response.flatMap { response ->
        ...
        if (chatStatus == null || !chatStatus.retryRequired) {
            ...
            for (e2eKey in chatStatus.keyList!!) {
              if (e2eKey.returnCode != null && e2eKey.returnCode.equals("receiverKeyNotValid")) {
                ...
                if (e2eKey.deviceId == deviceId) {
                  ekeyDao.setKeyId(
                    preference.chatUserId, e2eKey.publicKey, e2eKey.keyId, eKeyTable.time, deviceId
                  )
                } else {
          ...
          if (response.replyThreadFeatureData?.parentMsgId != null && response.replyThreadFeatureData?.parentMsgId!!.isNotEmpty()) {
            ...
            val chatThread = dataManager.db().chatThreadDao().getChatByClientId(msgId1, threadId)
            ...
            if (response.replyThreadFeatureData?.replyMsgConfig == 1) {
              val singleChat =
                dataManager.db().singleChatDao().getChatByLocalMsgId(msgId1, threadId)
      ...
      networkManager.api()
        .editMessage(
          ...
          if (msgUpdateResponse.replyThreadFeatureData != null) {
            val chatThread =
              chatThreadDao.getChatByServerMsgId(msgUpdateResponse.msgUniqueId, threadId)
            ...
            val reactionsFromDatabase = chatReactionDao.getAllThreadReactions(chatThread.id).blockingGet()
            val reactionRecords = composeReactionList(reactionsFromDatabase)
            ...
            if (msgUpdateResponse.replyThreadFeatureData!!.replyMsgConfig == 1) {
              ...
              Single.just(transform(singleChat = copy, chatReactions = reactionRecords))
            } else {
              Single.just(transform(chatThread = chatThread, chatReactions = reactionRecords))
            }
          } else {
            val singleChat =
            ...
            Single.just(transform(singleChat = singleChat, chatThreadList = chatThreads, chatReactions = reactionRecords))
          }
        }
    ...
    return Single.create { e: SingleEmitter<Thread?> ->
      ...
      networkManager.api()
        .deleteMessage(
          tenantId,
          chatUserId,
          MessageRequest(chats = messageListRequest, threadId = threadId, deleteType = deleteType)
          ...
          data?.size?.let {
            0.until(it).forEach { i ->
              ...
              if (data[i].replyThreadFeatureData != null) {
                val chatThread = chatThreadDao.getChatByClientId(messageList[i].parentMsgId, threadId)

                if (data[i].replyThreadFeatureData!!.replyMsgConfig == 1) {
                  val singleChatParent =
                    singleChatDao!!.getChatByServerMsgId(data[i].msgUniqueId, threadId)
                  if (deleteType == DeleteType.DELETE_FOR_EVERYONE.type) {
                    singleChatParent.message = Constants.Features.THIS_MESSAGE_WAS_DELETED
                    singleChatParent.deleteType = DeleteType.DELETE_FOR_EVERYONE.type
                  } else {
                    singleChatParent.message = Constants.Features.THIS_MESSAGE_WAS_DELETED_FOR_YOU
                    singleChatParent.deleteType = DeleteType.DELETE_FOR_USER.type
                  }
                  ...
                  reactionDao.clearAllThreadUserReaction(chatThread.id, chatUserId)
                ...
                when (deleteType) {
                  DeleteType.DELETE_FOR_EVERYONE.type -> {
                    chatThread.message = Constants.Features.THIS_MESSAGE_WAS_DELETED
                    chatThread.deleteType = DeleteType.DELETE_FOR_EVERYONE.type
                  }
                  else                                -> {
                    chatThread.deleteType = DeleteType.DELETE_FOR_USER.type
                    chatThread.message = Constants.Features.THIS_MESSAGE_WAS_DELETED_FOR_YOU
                  }
                }
          ...
          networkManager.api()
            ...
            .flatMap {
              ...
              val chatThreadReactions =
                chatReactionDao.getChatThreadReactionsCount(reaction.msgId, reaction.emojiCode)
                  .blockingGet()
        ...
        networkManager.api()
          ...
          .flatMap {
            if (reaction.chatReactionType == ChatReactionType.SET) {
              ...
              chatReactionDao.clearChatUserReaction(singleChat.id, reaction.emojiCode, chatUserId)
            }

            val chatThreadReactions =
              chatReactionDao.getChatReactionsCount(reaction.msgId, reaction.emojiCode)
                .blockingGet()
            val chatReactionRecord = composeReactionList(chatThreadReactions)
            val userRecordList = if (chatReactionRecord.isNullOrEmpty()) {
              null
    ...
    return Single.create {
      ...
      if (chatType == ChatType.SINGLE || chatType == ChatType.GROUP) {
        ...
        if (message.file != null || message.media != null) {
          ...
          mediaThumbnailPath = preference.chatServer?.let { it1 -> mediaThumbnailPath?.replace(it1, "") }
        ...
        if (userList.isNotEmpty()) {
          userList.forEach { user ->
            if (isE2E) {
              ...
              var threadId = dataManager.db().threadDao().getThreadIdByRecipientId(user.id).blockingGet()
              if (threadId == null) {
      ...
      BiFunction { singleChatList: List<SingleChat>, chatThreadList: List<ChatThread> ->
        ...
        for (singleChat in singleChatList) {
          ...
          if (singleChat.type.equals(ChatType.GROUP.type)) {
            groupRecord = GroupMapper.transform(
              dataManager.db().groupDao().getGroupByThreadId(tenantId, singleChat.threadId),
              tenantId,
              appUserId
            )
          } else {
            val userId = dataManager.db().threadDao().getUserIdByThreadId(singleChat.threadId)
            if (eventType == INCOMING) {
              senderUserRecord = UserMapper.transform(userDao.getUserByIdInSync(tenantId, userId))
            } else if (eventType == OUTGOING) {
              receiverUserRecord = UserMapper.transform(userDao.getUserByIdInSync(tenantId, userId))
            }
          }
        ...
        for (chatThread in chatThreadList) {
          ...
          if (chatThread.type.equals(ChatType.GROUP_CHAT_THREAD.type)) {
            groupRecord = GroupMapper.transform(
              dataManager.db().groupDao().getGroupByThreadId(tenantId, chatThread.threadId),
              tenantId,
              appUserId
            )
          } else {
            val userId = dataManager.db().threadDao().getUserIdByThreadId(chatThread.threadId)
            if (eventType == INCOMING) {
              senderUserRecord = UserMapper.transform(userDao.getUserByIdInSync(tenantId, userId))
            } else if (eventType == OUTGOING) {
              receiverUserRecord = UserMapper.transform(userDao.getUserByIdInSync(tenantId, userId))
  ...
  override fun globalSearch(
    ...
    return networkManager.api().globalSearch(tenantId, chatUserId, SearchRequest(SearchQuery(searchedText, threadId), listOf("messages")))
            ...
            if (searchedMessageResponse.threadId.equals(threadResponse.threadId)) {
              if (threadResponse.user != null) {
                if (threadResponse.threadType == ChatType.GROUP.type) {
                  ...
                  groupRecord = GroupMapper.transform(
                    dataManager.db().groupDao().getGroupByThreadId(tenantId, threadResponse.threadId),
                    tenantId,
                    chatUserId
                  )
                } else {
        ...
        .reportMessage(
          ...
          if (messageRecord.chatThreadMetadata != null) {
            if (messageRecord.chatThreadMetadata!!.sendToChannel == 1) {
              ...
              val reactionsFromDatabase = chatReactionDao.getAllReactions(singleChat.id).blockingGet()
              val reactionRecords = composeReactionList(reactionsFromDatabase)
              Single.just(transform(singleChat = singleChat, chatReactions = reactionRecords))
            }
    ...
    return Single.create { e: SingleEmitter<Thread?> ->
      ...
      response = if (parentMsgId != null && parentMsgId.isNotEmpty()) {
        ...
        if (singleChat.sendToChannel != null && singleChat.sendToChannel == 1) {
          val singleChatParent = singleChatDao.getChatByLocalMsgId(singleChat.parentMsgId, threadId)
  ...
  override fun getMediaGallery(
    ...
    return remote.getMediaGallery(tenantId, threadId, msgType, currentMsgId, direction, pageSize)
  }
  ...
  override fun forwardMediaChat(
    ...
    return Single.create {
      ...
      val apiKey = dataManager.preference().apiKey
      val response =
        networkManager.api().forwardChat(apiKey!!, tenantId, chatUserId, messageRequests).blockingGet()
      if (response.messageResponse.isNullOrEmpty()) {
        it.onSuccess(Result(false))
      } else {
        response.messageResponse.forEach { messageResponse ->
  ...
  private fun userBlockStatusUpdated(user: User) {
    ...
    userWrapper
      ...
      .subscribe(CrashReportingObserver<UserRecord>(disposableList))
  }
