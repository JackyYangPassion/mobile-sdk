...
import com.ripbull.coresdk.announcement.mapper.AnnouncementMapper
import com.ripbull.coresdk.announcement.mapper.AnnouncementRecord
import com.ripbull.coresdk.chat.mapper.ChatEntityMapper.getChatRow
import com.ripbull.coresdk.chat.mapper.ChatEntityMapper.getGroupInfoChatRow
import com.ripbull.coresdk.chat.mapper.ChatEntityMapper.getThreadChatRow
import com.ripbull.coresdk.chat.mapper.ChatEvent
import com.ripbull.coresdk.chat.mapper.ChatReactionRecord
import com.ripbull.coresdk.chat.mapper.ChatRecordMapper.transform
...
import com.ripbull.coresdk.utils.Constants.Features.DELETE
import com.ripbull.coresdk.utils.Constants.Features.EDIT
import com.ripbull.coresdk.utils.Constants.Features.SELF
import com.ripbull.coresdk.utils.Constants.Features.THIS_MESSAGE_WAS_DELETED
import com.ripbull.coresdk.utils.Constants.Features.THIS_MESSAGE_WAS_DELETED_FOR_YOU
import com.ripbull.coresdk.utils.Logger.d
import com.ripbull.coresdk.utils.Logger.i
...
import com.ripbull.ertc.mqtt.model.ChatUpdateResponse
...
class ChatWrapper(private val dataManager: DataManager, private val eventHandler: EventHandler) {
  ...
  fun messagesOn(message: ReceivedMessage): Observable<MessageRecord?> {
    return Observable.create { e: ObservableEmitter<MessageRecord?> ->
      ...
      response.isFollowThread = if (response.follow == true) { 1 } else { 0 }
      ...
      if (threadUserLinks.size > 0) {
        if (replyThreadFeatureData?.baseMsgUniqueId != null) { // update thread table
          ...
          if (response.thread.threadType == ChatType.GROUP.type) {
            val group = db.groupDao().getGroupByThreadId(response.tenantId, response.thread.threadId)
            groupRecord = GroupMapper.transform(group, response.tenantId, getAppUserId())
          }
          ...
          val singleChatParent = db.singleChatDao()
            .getChatByServerMsgId(response.replyThreadFeatureData!!.baseMsgUniqueId, threadId)

          if (chatType == ChatEventType.INCOMING) {
            //increasing the unreadCount of a thread
            val unreadCount = db.threadDao().getThreadByIdInSync(threadId).unReadCount + 1
          ...
          Observable.create { e: ObservableEmitter<MessageRecord?> ->
            ...
            e.onNext(
              transform(
                ...
                notificationRecord = NotificationRecord(message.tile(), message.body()),
                chatEventType = chatType,
                isSilent = response.isSilent
              )
            )
            val chatThreads = db.chatThreadDao().getChatThreads(singleChatParent.id).blockingGet()

            if (replyThreadFeatureData.replyMsgConfig == 1) {
              val singleChat = getChatRow(
                response, ThreadMapper.from(response, getChatUserId(), getAppUserId()), preferenceManager,
                db.ekeyDao(), singleChatParent.id, getParentMessage(singleChatParent), msgId = msgId
              )
          ...
          val senderUserRecord: UserRecord? = if (response.thread.threadType == ChatType.GROUP.type) {
            UserMapper.transform(
              db.userDao().getUserByIdInSync(
                response.tenantId, response.sender.appUserId
              )
        ...
        if (response.thread.threadType == ChatType.SINGLE.type) { // create thread & then message table
          ...
          val from =
            ThreadMapper.from(response, chatUserId, appUserId, recipientChatUserId, recipientAppUserId)
          ...
          val senderUserRecord =
            UserRecord(id = response.sender.appUserId, name = response.sender.name)
          db.userDao().updateUserChatId(response.sender.appUserId, response.sender.eRTCUserId)
          ...
          if (keyDetails != null) {
            ...
            val updatedRow: Int = db.ekeyDao().updateKey(
              eKeyTable1.ertcUserId, eKeyTable1.publicKey, eKeyTable1.keyId, eKeyTable1.deviceId, System.currentTimeMillis()
            )
          ...
          dataManager.network()
            ...
            .getGroupByThreadId(
              ...
              val thread =
                ThreadMapper.fromGroupResponse(groupResponse, getAppUserId(), getChatUserId())
              if (chatType == ChatEventType.INCOMING) {
              ...
              val chatRow =
                getChatRow(response, thread, preferenceManager, db.ekeyDao(), msgId = msgId)
            ...
            .subscribe({
              ...
              val group = db.groupDao().getGroupByThreadId(response.tenantId, response.thread.threadId)
              val groupRecord = GroupMapper.transform(group, response.tenantId, getAppUserId())
              e.onNext(
                transform(
                  singleChat = it,
                  senderRecord = senderUserRecord,
    ...
    }.doOnError { error -> i("messageOn", "error: " + error.message) }.subscribeOn(Schedulers.io())
  }
      ...
      if (status.replyThreadFeatureData != null &&
        ...
        if (status.replyThreadFeatureData!!.replyMsgConfig == 1) {
          ...
          Observable.create { e: ObservableEmitter<MessageRecord?> ->
            ...
            val singleChat = singleChatDao.getChatByServerMsgId(status.msgUniqueId, status.threadId)
            ...
            val reactionsFromDatabase = reactionDao.getAllReactions(singleChat.id).blockingGet()
            val reactionRecords = composeReactionList(reactionsFromDatabase)
            e.onNext(transform(singleChat = singleChat, chatEventType = eventType, chatReactions = reactionRecords))
          }.subscribeOn(Schedulers.io()).doOnNext { it?.let { it1 -> e.onNext(it1) } }.subscribe()
        ...
        Observable.create { e: ObservableEmitter<MessageRecord?> ->
          ...
          val reactionsFromDatabase = reactionDao.getAllThreadReactions(chatThread.id).blockingGet()
          val reactionRecords = composeReactionList(reactionsFromDatabase)
          e.onNext(transform(chatThread = chatThread, chatEventType = eventType, chatReactions = reactionRecords))
        }.subscribeOn(Schedulers.io()).doOnNext { it?.let { it1 -> e.onNext(it1) } }.subscribe()
      ...
      status.chats?.forEach { it ->
        ...
        if (it.replyThreadFeatureData != null) {
          if (it.replyThreadFeatureData!!.replyMsgConfig == 1) {
            val singleChatDao = db.singleChatDao()
            val singleChat = singleChatDao.getChatByServerMsgId(it.msgUniqueId, threadId)
            d("thread_debug ###", "singleChat : $singleChat")
            var senderUserRecord: UserRecord? = null
            if (singleChat.type.equals(ChatType.GROUP.type)) {
              senderUserRecord = UserMapper.transform(
                db.userDao().getUserByIdInSync(getTenantId()!!, singleChat.senderAppUserId)
              )
            }
            ...
            if (it.message == null && status.updateType != DELETE) {
              ...
              it.message?.let { message ->
                if (isE2EFeatureEnabled()) {
                  ...
                  val publicKey = Objects.requireNonNull(status.senderKeyDetails)?.publicKey
                  val msg =
                    ECDHUtils.decrypt(Objects.requireNonNull(message), publicKey, privateKey)
          ...
          if (chatThread.type.equals(ChatType.GROUP_CHAT_THREAD.type)) {
            senderUserRecord = UserMapper.transform(
              db.userDao().getUserByIdInSync(getTenantId()!!, chatThread.senderAppUserId)
            )
          }
          val eventType = if (chatThread.senderAppUserId != getAppUserId()) {
            ChatEventType.INCOMING
        ...
        if (deliveredMessages != null) {
          for (chatThread in deliveredMessages) {
            ...
            publishStatus(
              ...
              chatThread.msgUniqueId?.let { ReplyThread(it, chatThread.sendToChannel) }
            )
    ...
    return Observable.create { e: ObservableEmitter<GroupRecord> ->
      ...
      groupUpdate.eventList?.let {
        if (it.isNotEmpty() && it[0].eventType.equals(GroupRecord.STATUS_DELETED)) {
          dataManager.db().groupDao().updateGroupStatus(groupUpdate.groupId, GroupRecord.STATUS_DELETED)
          ...
          dataManager.db().groupDao().updateGroupStatus(groupUpdate.groupId, GroupRecord.STATUS_DEACTIVATED)
          val group = db.groupDao().getGroupById(groupUpdate.groupId)
          e.onNext(GroupMapper.transform(group, group.tenantId, getAppUserId()))
          db.groupDao().deleteByGroupId(groupUpdate.groupId)
          db.chatReactionDao().deleteByThreadId(groupUpdate.threadId)
  ...
  fun groupUpdatedMessage(receivedMessage: ReceivedMessage): Observable<MessageRecord?> {
    return Observable.create { e: ObservableEmitter<MessageRecord?> ->
      ...
      groupUpdate.eventList?.let {
        if (it.isNotEmpty() && it[0].eventType.equals(GroupRecord.STATUS_DELETED)) {
          return@create
        } else if (it.isNotEmpty() && (it[0].eventType.equals("frozen") || it[0].eventType.equals("unfrozen"))) {
          return@create
        } else if (it.isNotEmpty() && (it[0].eventType.equals("activated") || it[0].eventType.equals("deactivated"))) {
          return@create
        }
      }

      val threadId = groupUpdate.threadId
      val group = preferenceManager.tenantId?.let { db.groupDao().getGroupByThreadId(it, threadId) }
      ...
      if (group == null) {
            ...
            groupUpdate.groupId?.let { it2 ->
              dataManager.network()
                ...
                .map { groupResponse: GroupResponse ->
                  ...
                  val threadUserLinks = threadId?.let { db.threadUserLinkDao().hasThread(it) }
                  ...
                  if (!TextUtils.isEmpty(eventUser)) {
                    for (event: Event in groupUpdate.eventList!!) {
                      ...
                      d("ChatWrapper", "if ==> chatRow message: " + chatRow?.message?.trim())
                      val lastMessage = db.singleChatDao().getLastGroupEventMessage(ChatEventType.CHAT_META_DATA.type, thread.id)
        ...
        val eventUser: String? = groupUpdate.eventTriggeredByUser?.appUserId
        val chatEventType = if (getAppUserId().equals(groupUpdate.eventTriggeredByUser?.appUserId)) {
          ChatEventType.OUTGOING
        } else {
          ChatEventType.INCOMING
        }
  ...
  private fun getDownloadMessageRecord(
    ...
    when {
      singleChat != null -> {
        ...
        return transform(
          singleChat = singleChat, senderRecord = senderUserRecord,
          chatThreadList = singleChatEmbedded.listChatThread, chatEventType = chatEventType
        )
      }
      chatThread != null -> {
        chatThread.downloadStatus = downloadStatus
  ...
  private fun isE2EFeatureEnabled(): Boolean {
    return (dataManager.db().tenantDao().getFeatureEnabled(dataManager.preference().tenantId, com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT) == "1")
  }
  ...
  fun announcementTrigger(receivedMessage: ReceivedMessage): Observable<AnnouncementRecord> {
    return Observable.create { e: ObservableEmitter<AnnouncementRecord> ->
      val announcement = gson.fromJson(receivedMessage.message, AnnouncementResponse::class.java)
  ...
  private fun getConfigList(
    ...
    configArrayList.add(
      TenantConfig(
        tenant.id,
        com.ripbull.coresdk.utils.Constants.TenantConfig.CHAT_ENABLED,
    ...
    configArrayList.add(
      TenantConfig(
        tenant.id,
        com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_ENABLED,
    ...
    configArrayList.add(
      TenantConfig(
        tenant.id,
        com.ripbull.coresdk.utils.Constants.TenantConfig.TYPING_STATUS,
    ...
    if (readReceipt != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.READ_RECEIPTS,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ReadReceipts.SENT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ReadReceipts.DELIVERED,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ReadReceipts.READ,
    ...
    if (features.userProfile != null) {
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.USER_PROFILE,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.NAME,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.Name.EDITABLE,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.IMAGE,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.Image.EDITABLE,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.FAV_CONTACTS,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.AVAILABLE_STATUS,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.AvailableStatus.ALLOW_OVERRIDING,
    ...
    if (features.blockUser != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.BLOCK_USER,
    ...
    if (features.starredChat != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.STAR_MESSAGE,
    ...
    if (features.notificationSettings != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.NOTIFICATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SingeChat.Notification.MUTE_SETTINGS,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GroupChat.Notification.MUTE_SETTINGS,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.UserProfile.Notification.MUTE_SETTINGS,
    ...
    if (features.searchFilter != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SEARCH_FILTER,
    ...
    if (chatAttachment != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ATTACHMENT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_AUDIO,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_VIDEO,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_LOCATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_CONTACT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_DOCUMENT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_IMAGE,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.SINGLE_CHAT_GIPHY,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_AUDIO,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_VIDEO,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_LOCATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_CONTACT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_DOCUMENT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_IMAGE,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GROUP_CHAT_GIPHY,
    ...
    if (forwardChat != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.FORWARD_CHAT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.SINGLE_CHAT_TEXT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.SINGLE_CHAT_MEDIA,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.SINGLE_CHAT_LOCATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.SINGLE_CHAT_CONTACT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.SINGLE_CHAT_GIPHY,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.GROUP_CHAT_TEXT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.GROUP_CHAT_MEDIA,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.GROUP_CHAT_LOCATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.GROUP_CHAT_CONTACT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ForwardChat.GROUP_CHAT_GIPHY,
    ...
    if (editChat != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.EDIT_CHAT,
    ...
    if (e2eEncryption != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT_TEXT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT_MEDIA,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT_LOCATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT_CONTACT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.E2E_CHAT_GIFY,
    ...
    if (deleteChat != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.DELETE_CHAT,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.DeleteChat.DELETE_FOR_SELF,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.DeleteChat.DELETE_FOR_EVERYONE,
          if (deleteChat.deleteForEveryone!!.enabled!!) "1" else "0"
        )
      )
    } else {
      configArrayList.add(
        TenantConfig(tenant.id, com.ripbull.coresdk.utils.Constants.TenantConfig.DELETE_CHAT, "0")
      )
      configArrayList.add(
        TenantConfig(
          tenant.id,
    ...
    configArrayList.add(
      TenantConfig(
        tenant.id,
        com.ripbull.coresdk.utils.Constants.TenantConfig.CHAT_REACTIONS,
    ...
    configArrayList.add(
      TenantConfig(
        tenant.id,
        com.ripbull.coresdk.utils.Constants.TenantConfig.REPLY_THREAD,
    ...
    if (features.followChat != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.FOLLOW_CHAT,
    ...
    if (unifiedSearch != null) {
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.CHANNEL_SEARCH,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.GLOBAL_SEARCH,
          if (features.unifiedSearch!!.enabled!!) "1" else "0"
        )
      )
    }
    ...
    if (userMentions != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.USER_MENTIONS,
    ...
    if (features.announcements != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.ANNOUNCEMENT,
    ...
    if (features.copyChat != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.COPY,
    ...
    if (features.localSearch != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.LOCAL_SEARCH,
    ...
    if (moderation != null) {
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.MODERATION,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.DOMAIN_FILTER,
      ...
      configArrayList.add(
        TenantConfig(
          tenant.id,
          com.ripbull.coresdk.utils.Constants.TenantConfig.PROFANITY_FILTER,
